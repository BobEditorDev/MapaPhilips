...
class TerreoRooms {
    ...
    setup() {
        try {
            ...
            this.bindEvents();
            this.showAllRooms();
            console.log('[share] TerreoRooms inicializado com sucesso. window.terreoRooms =', window.terreoRooms);
        } catch (err) {
            console.error('[share] ERRO na inicialização de TerreoRooms:', err);
        }
    }
    ...
    filterRooms(query) {
        ...
        if (searchTerm === '') {
            ...
            // MOTIVO: Usuários podem buscar por "T-MR01", "Wireless" ou "10" (pessoas)
    ...
    renderRoomsList() {
        ...
        if (this.filteredRooms.length === 0) {
            ...
            // Atualiza contador com zero resultados
        ...
        const roomsHTML = sortedRooms.map(room => {
            ...
            // Detecta se a sala está indisponível (Fechada ou Trancada)
...
Biografia: ${biografiaTexto}`;
    ...
    handleKeyboardNavigation(e) {
        ...
        if (e.key === 'Escape') {
            ...
            if (searchInput && searchInput.value !== '') {
                // O keydown do campo de busca já vai tratar, se o foco estiver lá
                // Mas se o Escape veio de outro elemento com busca ativa, limpa também
    ...
    showSharedPin(naturalX, naturalY) {
        ...
        // Remover pin anterior se houver
